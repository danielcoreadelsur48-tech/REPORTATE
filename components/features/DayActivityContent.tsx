...
export function lastSeenKey(groupId: string) {
...
interface DayActivityContentProps {
  groupId: string | null;
  isCaptain: boolean;
  isActive: boolean;
}

export function DayActivityContent({ groupId, isCaptain, isActive }: DayActivityContentProps) {
  ...
  useEffect(() => {
    ...
    channelRef.current = supabase
      .channel(`activity_content_${groupId}`)
      .on(
        'postgres_changes',
      ...
      <View style={[styles.row, { borderBottomColor: borderColor }]}>
        ...
        <View style={styles.rowContent}>
          <View style={styles.rowTop}>
            <Text
              style={[styles.rowName, { color: textColor, fontWeight: unread ? Typography.weight.bold : Typography.weight.regular }]}
              numberOfLines={1}
            >
              {item.userFullName}
            </Text>
  ...
  function renderSOSItem({ item }: { item: DaySOSItem }) {
    ...
    const sosIsActive = item.status === 'active';
    return (
      <View style={[styles.row, { borderBottomColor: borderColor }]}>
        <View style={styles.unreadDotWrap}>
          {unread && <View style={styles.unreadDot} />}
        ...
        <View style={styles.rowContent}>
          ...
          <View style={styles.rowSub}>
            <Badge
              label={sosIsActive ? STRINGS.ACTIVITY_SHEET.SOS_ACTIVE : STRINGS.ACTIVITY_SHEET.SOS_RESOLVED}
              variant={sosIsActive ? 'danger' : 'neutral'}
            />
          </View>
        </View>
        {item.location && (
        ...
        {tabs.map((tab) => {
          ...
          return (
            <TouchableOpacity
              ...
              activeOpacity={0.75}
            >
              <Text style={[styles.tabLabel, { color: active ? Colors.text.inverse : Colors.text.secondary }]}>
                {tab.label}
              </Text>
            </TouchableOpacity>
          );
